...
Hash::MergeX - Merges arbitrarily deep hashes into a single hash
...
=head1 SYNOPSIS
	...
	Hash::MergeX::specify_behavior(
	    {
			...
			'HASH' => {
				'SCALAR' => sub { $_[1] },
				'ARRAY'  => sub { [ values %{$_[0]}, @{$_[1]} ] },
				'HASH'   => sub { Hash::MergeX::_merge_hashes( $_[0], $_[1] ) }, 
			},
		}, 
		'My Behavior', 
	);
	...
	my $merge = Hash::MergeX->new( 'LEFT_PRECEDENT' );
...
Hash::MergeX merges two arbitrarily deep hashes into a single hash.  That
...
values when keys are conflicting, Hash::MergeX provides several preset
...
attempted.
   ...
   Hash::MergeX::set_set_behavior('LEFT_PRECEDENT')
...
hash values never being lost
   ...
   my $merge = Hash::MergeX->new('RIGHT_PRECEDENT');
   $merge->set_set_behavior('RIGHT_PRECEDENT')
   Hash::MergeX::set_set_behavior('RIGHT_PRECEDENT')
...
the other, but if this isn't possible, the data is dropped.
   ...
   my $merge = Hash::MergeX->new('STORAGE_PRECEDENT');
   $merge->set_set_behavior('STORAGE_PRECEDENT')
   Hash::MergeX::set_set_behavior('STORAGE_PRECEDENT')
...
a hash.
   ...
   my $merge = Hash::MergeX->new('RETAINMENT_PRECEDENT');
   $merge->set_set_behavior('RETAINMENT_PRECEDENT')
   Hash::MergeX::set_set_behavior('RETAINMENT_PRECEDENT')
...
Sets how the data cloning is handled by Hash::MergeX.  If this is true,
...
Returns the behavior that is currently in use by Hash::MergeX.

=item specify_behavior( <hashref>, [<name>] )

Specify a custom merge behavior for Hash::MergeX.  This must be a hashref
